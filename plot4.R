...
# plot 4.1
plot(data_power$Date, data_power$Global_active_power, type = "line", xlab = "", ylab = "Global Active Power", xaxt = "n" )
axis(1, at = date_label$Date, labels = substr(weekdays(date_label$Date),1,3))

# plot 4.2
plot(data_power$Date, data_power$Voltage, type = "line", xlab = "datetime", ylab = "Voltage", xaxt = "n" )
axis(1, at = date_label$Date, labels = substr(weekdays(date_label$Date),1,3))
...
# plot 4.4
with(data_power, plot(Date, Global_reactive_power, xlab="datetime ", type = "line", xaxt = "n"))
axis(1, at = date_label$Date, labels = substr(weekdays(date_label$Date),1,3))

dev.copy(png, filename = "plot4.png", width = 480, height = 480,res = 72)
dev.off()

# back to normal graph view 
par(mfrow = c(1,1))